...
namespace BMT_backend.Tests.Services
{
    public class CompletedOrdersReport
    {
        ...
        [Test]
        public async Task GetOrderReportsAsync_WhenStatusInicialIs4_CallsFormatCompletedOrders()
        {
            // Arrange
            ...
            Assert.That(result[0].DateReceived, Is.EqualTo("2023-02-20"));
            Assert.That(result[0].ProductCost, Is.EqualTo(5000));
            Assert.That(result[0].FeeCost, Is.EqualTo(500));
            Assert.That(result[0].TotalCost, Is.EqualTo(5500));
        }
    }
}